...
auth.onAuthStateChanged(user => {
	...
	if(platform.manufacturer !== null) {
		anonP.innerHTML = `
		Bank logs can be sent via <span>email / sms </span>, as a dynamic link that expires after 7 hours and also downloaded on your 
		<span>${platform.manufacturer} ${platform.product} ${platform.os}</span> Downloads folder
		`
	} else {
		anonP.innerHTML = `
		Bank logs can be sent via <span>email / sms </span>, as a dynamic link that expires after 7 hours and also downloaded on your 
		<span>${platform.os}</span> from <span>${platform.name}</span> browser 
		`
	}
});
const emailVerifyBtn = document.getElementById('email-verify-btn');
emailVerifyBtn.addEventListener('click', sendEmail);
